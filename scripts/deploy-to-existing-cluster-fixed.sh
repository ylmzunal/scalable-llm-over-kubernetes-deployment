...
# Update the cloud deployment file with correct image URI
print_status "Updating deployment configuration..."
sed "s|gcr.io/PROJECT_ID/llm-chatbot-backend:latest|$IMAGE_URI|g" k8s/backend-deployment-cloud.yaml > /tmp/backend-deployment-cloud.yaml
...
print_status "Image deployed: $IMAGE_URI" 